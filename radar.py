...
def check_access():
    # ── Step 1: Token URL se lo ya session se lo ──────────
    params = st.query_params
    token = params.get("token", "")

    # Agar URL mein token hai to session mein save karo
    if token:
        st.session_state["auth_token"] = token
    # Agar URL mein nahi hai to session se lo
    ...
    # ── Step 3: Token URL mein keep karo har rerun par ────
